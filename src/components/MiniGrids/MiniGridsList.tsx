...
export default function MiniGridsList() {
  ...
  // Enrichir minigrids avec site info quand rawMiniGrids ou sites changent
  useEffect(() => {
    if (sites.length > 0) {
...
// 🔹 Ajout d'une mini-grid
const handleAddGrid = async (e: React.FormEvent) => {
  e.preventDefault();
  setAddLoading(true);
  try {
    if (!addForm.nom || !addForm.site_id) throw new Error('Nom et site requis');
    await api.minigrids.create({
      site_id: Number(addForm.site_id),
      nom: addForm.nom,
      statut: addForm.statut,
    });
    refresh(); // Rafraîchir le contexte pour mettre à jour partout
  ...
  const filteredGrids = minigrids.filter(grid => {
    const matchesSearch = grid.nom.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  // 🔹 Styles visuels selon le statut
  ...
  const getCardBackground = (statut: string) => {
    switch (statut) {
      case 'en_service': return 'bg-green-50';
      case 'maintenance': return 'bg-orange-50';
      case 'hors_service': return 'bg-red-50';
      case 'projete': return 'bg-blue-50';
  ...
  if (contextLoading || loading) return <div className="p-6">Chargement…</div>;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Formulaire d'ajout */}
      {showAddForm && (
        <form onSubmit={handleAddGrid} className="bg-white border border-gray-200 rounded-lg p-4 flex flex-col md:flex-row gap-4 items-end">
          ...
          <div className="flex-1">
            ...
            >
              ...
              {sites.map(s => (
                <option key={s.id} value={s.id}>{s.localite || `Site #${s.id}`}</option>
              ))}
            </select>
          </div>
          <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredGrids.map(grid => (
          <div key={grid.id} className={`rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow ${getCardBackground(grid.statut)}`}>
            <div className="p-6">
              {/* Header */}
